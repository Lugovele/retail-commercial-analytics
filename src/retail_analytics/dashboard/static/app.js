...
const salesDriverBuckets = [
  ...
  { title: "Присутствие", concepts: ["selling_store_count", "active_store_count", "distribution", "numeric_distribution_store_format"] },
...
const portfolioPresentationFallback = {
  numeric_distribution_store_format: {
    display_label: "Дистрибуция в формате ТТ",
    format: "percent",
    unit_label: "%",
    business_meaning: "Доля торговых точек выбранного формата, где есть наблюдаемые продажи выбранного объекта.",
    business_question: "В какой доле ТТ выбранного формата товар, бренд, производитель или категория имеет продажи?",
    decision_use: "Показывает присутствие внутри конкретного формата без замены сетевой дистрибуции и без трактовки как листинг или OOS.",
...
const comparisonLabels = {
  ...
  CUSTOM: "Выбранный месяц",
  AVAILABLE_MONTH_SET: "Совпадающие месяцы",
  NONE: "Нет сравнения"
};
const filterConfig = {
...
function bindDynamicControls() {
  ...
  document.getElementById("sales-driver-store-format")?.addEventListener("change", async (event) => {
    await applyScopeChange(async () => {
      state.salesDriverStoreFormat = event.target.value || "";
...
async function loadOptions({ deferSku = false } = {}) {
  ...
  params.set("period_mode", state.periodMode);
  const dateFrom = optionsDateFrom();
  const dateTo = optionsDateTo();
  if (dateFrom) params.set("date_from", dateFrom);
  if (dateTo) params.set("date_to", dateTo);
  const comparisonPeriod = selectedComparisonPeriod();
  if (comparisonPeriod) params.set("comparison_period_start", comparisonPeriod);
...
function populatePeriodSelects(resetPeriods) {
  ...
  setupPeriodSelect("period-b", defaultComparisonPeriod(latest) || periods[Math.max(0, periods.length - 2)] || latest, resetPeriods);
  populateMatchedYearSelects(resetPeriods);
  setupPeriodSelect("date-from", earliest, resetPeriods);
  setupPeriodSelect("date-to", latest, resetPeriods);
  if (resetPeriods) syncDefaultComparisonPeriod();
  updateMatchedMonthsPreview();
  updatePeriodSummary();
}
...
function setupYearSelect(id, selected, resetSelection, years) {
  const select = document.getElementById(id);
  if (!select) return;
  const previous = select.value;
...
function defaultComparisonPeriod(current) {
  ...
  return periods.includes(yoy) ? yoy : "";
}

function syncDefaultComparisonPeriod() {
  const select = document.getElementById("period-b");
...
async function runOverviewQuery() {
  ...
  try {
    ...
    const summaryPromise = postJson("/api/dashboard/query", summaryPayload);
    const chartPromise = postJson("/api/dashboard/query", chartPayload);
    const overviewPortfolioPromise = postJson("/api/dashboard/portfolio-market", buildOverviewPortfolioPayload());
...
function buildQueryPayload(grain, entityIds, metricConcepts) {
  ...
  return {
    ...
    comparison_period_start: selectedComparisonPeriod(),
...
function buildPortfolioMarketPayload() {
  ...
  return {
    ...
    comparison_period_start: state.periodMode === "AVAILABLE_MONTH_SET" ? null : selectedComparisonPeriod(),
    include_lineage: true,
    mart_build_id: retailer.default_mart_build_id,
    private_label_scope: document.getElementById("private-label-scope").value
  };
...
function buildSalesDriverStoreFormatOptionsPayload() {
  const retailer = selectedRetailer();
  return {
    retailer_id: retailer.retailer_id,
    source_id: retailer.source_id,
...
function buildSalesDriverStoreFormatDistributionPayload(grain = salesDriverSummaryGrain(), storeFormat = state.salesDriverStoreFormat) {
  if (!storeFormat) return null;
  if (!salesDriverGrainSupport[storeFormatDistributionConcept]?.includes(grain)) return null;
  const formatFilters = {
    ...selectedParentFiltersForGrain(grain),
    store_format: [storeFormat]
  };
  return {
    ...buildQueryPayload(grain, entityIdsForSalesDriverSummary(grain), [storeFormatDistributionConcept]),
    entity_filters: formatFilters
  };
}

function storeFormatOptionsFromResponse(response) {
  const seen = new Set();
  return (response?.metric_results || [])
    .filter((result) => result.metric_concept === "revenue")
    .map((result) => ({
      value: result.dimension_values?.store_format || result.entity_id,
      label: result.label || result.dimension_values?.store_format || result.entity_id
    }))
...
function salesDriverBackendConcepts(grain = salesDriverSummaryGrain()) {
  return salesDriverConcepts(grain).filter((concept) => concept !== storeFormatDistributionConcept);
}
...
function overviewKpiDefinitionsForGroup(groupId) {
  return overviewKpiDefinitions.filter((definition) => definition.group === groupId);
}

function renderKpiGroup(group, cardRenderer) {
  const section = document.createElement("section");
  section.className = `kpi-group kpi-group--${group.id} kpi-group--${group.visualTier}`;
  section.dataset.kpiGroup = group.id;
  const label = document.createElement("h3");
  label.className = "kpi-group-label";
  label.textContent = group.label;
  section.appendChild(label);
  const cards = document.createElement("div");
  cards.className = "kpi-group-cards";
  cards.replaceChildren(...overviewKpiDefinitionsForGroup(group.id).map(cardRenderer));
...
function renderSalesDriverMatrix() {
  ...
  salesDriverRows().forEach(({ group, concept, result, entry }) => {
    const response = salesDriverResponseForConcept(concept);
    const tr = document.createElement("tr");
    ...
    if (result && salesDriverMetricEntry(concept) && concept !== storeFormatDistributionConcept) {
    ...
    salesDriverMetricCells(result, entry, concept, response).forEach((cell) => {
      const td = document.createElement("td");
...
function renderSalesDriverStoreFormatControl() {
  const select = document.getElementById("sales-driver-store-format");
  const context = document.getElementById("sales-driver-store-format-context");
  if (!select || !context) return;
  const options = state.salesDriverStoreFormatOptions || [];
  select.replaceChildren(
    option("", "Сеть"),
    ...options.map((item) => option(item.value, item.label))
  );
  select.value = options.some((item) => item.value === state.salesDriverStoreFormat) ? state.salesDriverStoreFormat : "";
  const selectedLabel = options.find((item) => item.value === state.salesDriverStoreFormat)?.label;
  if (!state.salesDriverStoreFormat) {
    context.textContent = "Дистрибуция сети показана отдельно; выберите формат, чтобы увидеть знаменатель формата ТТ.";
    return;
  }
  context.textContent = selectedLabel
    ? `Знаменатель: все ТТ формата «${selectedLabel}» в месячной вселенной.`
    : "Знаменатель: все ТТ выбранного формата в месячной вселенной.";
}
...
function salesDriverMetricCells(result, entry, concept, response = state.salesDriversResponse) {
  ...
  if (localLimitation) {
    return isComparisonDisplayMode()
      ? [staticCell("Недоступно"), staticCell("Недоступно"), staticCell(localLimitation)]
      : [staticCell("Недоступно"), staticCell(localLimitation)];
  }
  if (!result || !entry) {
    return isComparisonDisplayMode()
...
function signalPeriodContextText() {
  ...
  if (state.periodMode === "AVAILABLE_MONTH_SET") return `Совпадающие месяцы · ${availableMonthSummaryText(state.summaryResponse || state.signalsResponse)}`;
  return `${formatPeriod(selectedDateFrom())} · ${comparisonLabels[selectedComparisonMode()]}`;
}
...
function updatePeriodSummary() {
  ...
  if (state.periodMode === "DATE_RANGE") {
    ...
    detail.textContent = "Диапазон";
    return;
  }
  if (state.periodMode === "SINGLE_PERIOD") {
    summary.textContent = formatCompactPeriod(selectedDateFrom()) || "Выберите период";
    detail.textContent = "Один месяц";
    return;
  }
  if (state.periodMode === "AVAILABLE_MONTH_SET") {
    summary.textContent = matchedMonthSummaryText() || "Выберите период";
    detail.textContent = "Совпадающие месяцы";
    return;
  }
  const reference = selectedComparisonPeriod();
  summary.textContent = `${formatCompactPeriod(selectedDateFrom())} / ${formatCompactPeriod(reference) || "нет периода"}`;
  detail.textContent = isDefaultYoyComparison() ? "Год к году" : "Выбранный месяц";
}
...
function updateAvailableMonthDisclosure(response) {
  ...
  if (!target) return;
  if (state.periodMode !== "AVAILABLE_MONTH_SET") {
    if (currentTarget) currentTarget.textContent = "Не используется";
    return;
  }
  ...
  if (currentTarget) {
    currentTarget.textContent = current || matchedMonthPreviewText() || availableMonthSummaryText(response);
    return;
  }
  target.textContent = current || matchedMonthPreviewText() || availableMonthSummaryText(response);
}
...
function updateMatchedMonthsPreview() {
  if (state.periodMode !== "AVAILABLE_MONTH_SET") return;
  const target = document.getElementById("available-months-current");
  if (target) target.textContent = matchedMonthPreviewText() || "Нет общих месяцев для сравнения";
}

function matchedMonthSummaryText() {
  const currentYear = document.getElementById("matched-current-year")?.value || "";
  const referenceYear = document.getElementById("matched-reference-year")?.value || "";
  const months = matchedMonthNames();
  if (!currentYear || !referenceYear) return "";
  const monthText = months.length ? months.join(", ") : "нет общих месяцев";
  return `${monthText} · ${currentYear} / ${referenceYear}`;
}

function matchedMonthPreviewText() {
  const months = matchedMonthNames();
  return months.length ? months.join(" · ") : "";
}

function matchedMonthNames() {
  const currentYear = Number(document.getElementById("matched-current-year")?.value || "");
  const referenceYear = Number(document.getElementById("matched-reference-year")?.value || "");
  if (!currentYear || !referenceYear) return [];
  const byYear = availableMonthNumbersByYear();
  const current = byYear.get(currentYear) || new Set();
  const reference = byYear.get(referenceYear) || new Set();
  return monthLabelsShort().filter((_, index) => current.has(index + 1) && reference.has(index + 1));
}

function availableMonthNumbersByYear() {
  const result = new Map();
  (state.options.periods || []).forEach((period) => {
    const date = new Date(`${period.value}T00:00:00`);
    const year = date.getFullYear();
    if (!result.has(year)) result.set(year, new Set());
    result.get(year).add(date.getMonth() + 1);
  });
  return result;
}
...
function selectedDateFrom() {
  ...
  if (state.periodMode === "AVAILABLE_MONTH_SET") return matchedYearBoundary("current", "start");
...
function selectedDateTo() {
  ...
  if (state.periodMode === "SINGLE_PERIOD") return document.getElementById("period-single")?.value || "";
  if (state.periodMode === "AVAILABLE_MONTH_SET") return matchedYearBoundary("current", "end");
  return document.getElementById("period-a")?.value || "";
}

function selectedComparisonMode() {
  if (state.periodMode === "COMPARE") return isDefaultYoyComparison() ? "YOY" : "CUSTOM";
  if (state.periodMode === "AVAILABLE_MONTH_SET") return "CUSTOM";
  return "NONE";
}
...
function optionsDateFrom() {
  const dates = [selectedDateFrom(), selectedComparisonPeriod()].filter(Boolean).sort();
  return dates[0] || "";
}

function optionsDateTo() {
  if (state.periodMode === "AVAILABLE_MONTH_SET") {
    const currentEnd = selectedDateTo();
    const referenceYear = Number(document.getElementById("matched-reference-year")?.value || "");
    const referenceEnd = referenceYear ? `${referenceYear}-12-01` : "";
    const dates = [currentEnd, referenceEnd].filter(Boolean).sort();
    return dates.at(-1) || "";
  }
  const dates = [selectedDateTo(), selectedComparisonPeriod()].filter(Boolean).sort();
  return dates.at(-1) || "";
}

function selectedComparisonPeriod() {
  if (state.periodMode === "COMPARE") return document.getElementById("period-b")?.value || "";
  if (state.periodMode === "AVAILABLE_MONTH_SET") return matchedYearBoundary("reference", "start");
  return "";
}

function isDefaultYoyComparison() {
  if (state.periodMode !== "COMPARE") return false;
  const current = document.getElementById("period-a")?.value || "";
  return Boolean(current) && selectedComparisonPeriod() === defaultComparisonPeriod(current);
}

function matchedYearBoundary(side, boundary) {
  const selectId = side === "reference" ? "matched-reference-year" : "matched-current-year";
  const year = Number(document.getElementById(selectId)?.value || "");
  if (!year) return "";
  return `${year}-${boundary === "end" ? "12" : "01"}-01`;
}
...
function salesDriverResultFor(concept) {
  if (concept === storeFormatDistributionConcept) {
    return state.salesDriversFormatDistributionResponse?.metric_results.find((result) => result.metric_concept === concept);
  }
  return state.salesDriversResponse?.metric_results.find((result) => result.metric_concept === concept);
}

function salesDriverResponseForConcept(concept) {
  return concept === storeFormatDistributionConcept
    ? state.salesDriversFormatDistributionResponse
    : state.salesDriversResponse;
}
...
function kpiContextText(comparison, entry) {
  ...
  if (state.periodMode === "AVAILABLE_MONTH_SET") return "За совпадающие месяцы";
  return "";
}
...
function salesDriverLocalLimitation(concept) {
  if (concept !== storeFormatDistributionConcept) return "";
  if (!state.salesDriverStoreFormat) return "Выберите формат ТТ, чтобы увидеть дистрибуцию в выбранном формате.";
  if (selectedStoreIds().length) return "Для выбранной ТТ дистрибуция по формату не поддерживается.";
  if (state.periodMode === "AVAILABLE_MONTH_SET") return "Для этого показателя сравнение по сопоставимым месяцам пока не поддерживается.";
  if (state.periodMode === "DATE_RANGE") return "Дистрибуция по формату доступна только по отдельным месяцам.";
  const limitations = state.salesDriversFormatDistributionResponse?.limitations || [];
  if (limitations.some((item) => item.issue_code === "store_format_distribution_period_mode_unsupported")) {
    return "Для выбранного режима периода дистрибуция по формату пока не поддерживается.";
  }
  if (limitations.some((item) => item.issue_code === "store_format_distribution_universe_empty")) {
    return "Для выбранного формата нет подтверждённой месячной вселенной ТТ.";
  }
  if (limitations.some((item) => item.issue_code === "store_format_distribution_requires_single_format")) {
    return "Выберите один формат ТТ для расчёта.";
  }
  if (limitations.length && !state.salesDriversFormatDistributionResponse?.metric_results?.length) {
    return "Для выбранного среза дистрибуция по формату пока не поддерживается.";
  }
  return "";
}
...
function periodContextText() {
  ...
  if (state.periodMode === "SINGLE_PERIOD") return formatPeriod(selectedDateFrom());
  if (state.periodMode === "AVAILABLE_MONTH_SET") {
    return `Совпадающие месяцы · ${availableMonthSummaryText(state.summaryResponse)}`;
  }
  const comparison = state.summaryResponse?.comparisons?.[0];
  const ref = comparison?.comparison_period_start ? formatPeriod(comparison.comparison_period_start) : "нет периода";
  return `${formatPeriod(selectedDateFrom())} vs ${ref} · ${comparisonLabels[selectedComparisonMode()]}`;
}
...
function provenanceSections(provenance, result, options = {}) {
  ...
  const sections = [
    ...
    section("Срез", [
      ...
      ...(periodSet.scope_type ? [["Режим периода", "Совпадающие месяцы"]] : []),
      ...
      ...(scope.entity_filters?.store_format?.length ? [["Формат ТТ", compactList(scope.entity_filters.store_format)]] : []),
      ["Учёт ассортимента", privateLabelScopeText(scope.private_label_scope)]
    ]),
    section("Расчёт", [
    ...
    section("Бизнес-правило", [
      ...
      ["Вселенная знаменателя", source.denominator_universe_type || value.share_scope || "н/д"]
    ]),
    section("Качество", [
      ["Статусы", compactList(quality.quality_statuses)],
...
function availableMonthAggregationLabel(method) {
  return {
    SUM_OF_MATCHED_AVAILABLE_MONTHS: "сумма совпадающих месяцев",
