...
| `weighted_shelf_price_vat` | Полочная цена | Средневзвешенная полочная цена с НДС из deterministic metric outputs. | currency | Показатели | Цена | DEFAULT_TABLE | network, category, manufacturer, brand, SKU, ТТ when available | Range value must be weighted from stored components, not arithmetic average. |
| `weighted_input_price_vat` | Входная цена | Средневзвешенная входная цена с НДС из deterministic metric outputs. | currency | Показатели | Цена | DEFAULT_TABLE | network, category, manufacturer, brand, SKU, ТТ when available | Range value must be weighted from stored components, not arithmetic average. |
...
| `distribution` | ND | Нумерическая дистрибуция на уровне периода; arbitrary selected-range recomputation не объявлен. | percent | Показатели | Присутствие | DEFAULT_TABLE | category, manufacturer, brand, SKU where defined | Full name: Нумерическая дистрибуция. Arbitrary range aggregation is not declared; do not average periods. |
| `numeric_distribution_store_format` | Дистрибуция в формате ТТ | Доля ТТ выбранного формата с наблюдаемыми продажами выбранного среза; знаменатель - месячная вселенная того же формата. | percent | Показатели | Присутствие | DEFAULT_TABLE | category, manufacturer, brand, SKU with exactly one selected store-format universe | Requires explicit store-format selection; Store filter, arbitrary range, available-month, region/FO/FO2/territory variants are unsupported. |
...
| `portfolio_package_mix` | Состав по упаковке | Portfolio mix grouped by canonical package/container attribute. | mixed | Портфель и рынок | Структура товара | RANKED_DECISION_ROW | package | Uses product-store metric facts joined to canonical attributes; no package ABC or flavor inference. |
| `portfolio_volume_mix` | Состав по объёму упаковки | Portfolio mix grouped by exact product-pack volume attribute. | mixed | Портфель и рынок | Структура товара | RANKED_DECISION_ROW | exact volume | Product-pack volume only, not sales volume; no invented volume bands or volume ABC. |
| `portfolio_package_volume_mix` | Состав по упаковке × объём | Compact mix grouped by canonical package and exact volume together. | mixed | Портфель и рынок | Структура товара | RANKED_DECISION_ROW | package × exact volume | Cardinality should be managed by local Portfolio mode, not permanent Excel-style columns. |
